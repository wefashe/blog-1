...
### 3.4 对调文章页前后篇的位置
文章页底部的时间较前的文章位置在左（主题中称之为 `next`），时间较后的文章位置在右（主题中称之为 `prev`）；这个不同于我习惯上的时间较前的文章位置在右（习惯上的 previous post），时间较后的文章位置在左（习惯上的 next post）。所以需要在 `themes/next/layout/_macro/post.swig` 中进行修改。

找到 `if not is_index and (post.prev or post.next)` 这一段，将这一段修改为以下内容，主要就是调整其中「next post」与「previous post」的链接与标题位置。
