...
### 3.4 对调文章页前后篇的位置
文章页底部的时间较前的文章位置在左，时间较后的文章位置在右；这个不同于我习惯上的时间较前的文章位置在右，时间较后的文章位置在左。所以需要在 `themes/next/layout/_macro/post.swig` 中进行修改。

找到 `if not is_index and (post.prev or post.next)` 这一段，将这一段修改为以下内容：
